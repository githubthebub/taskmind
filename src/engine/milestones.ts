import type { FocusMilestone } from '../types.js';
import type { EventBus } from '../state/bus.js';
import type { ProgressStore } from '../state/store.js';
...
export const MILESTONES: readonly FocusMilestone[] = [
...
export class MilestoneTracker {
  ...
  constructor(
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'hidden') this.breakFocus('visibility');
    });
    window.addEventListener('blur', () => this.breakFocus('blur'));
  }

  private breakFocus(reason: 'visibility' | 'blur'): void {
    if (!this.sessionActive || this.interrupted) return;
  ...
  private onCycleComplete(): void {
    if (this.interrupted) return; // Post-interruption cycles don't verify this session.
    this.runCycles += 1;
    ...
    const newlyReached = MILESTONES.filter(
      (m) => verifiedCycles >= m.cyclesRequired && !prev.milestonesReached.includes(m.id),
    );
    ...
    for (const m of newlyReached) this.bus.emit('milestone', m);
  ...
  finishSession(cycles: number): void {
    const prev = this.store.get();
  ...
  nextMilestone(): FocusMilestone | null {
