import type { FocusMilestone, ProgressState } from '../types.js';
...
export class Hud {
  ...
  private readonly cyclesEl: HTMLElement;
  ...
  constructor(container: HTMLElement) {
    ...
    `;
    this.levelEl = container.querySelector('[data-id="level"]') as HTMLElement;
    this.cyclesEl = container.querySelector('[data-id="cycles"]') as HTMLElement;
    this.bestEl = container.querySelector('[data-id="best"]') as HTMLElement;
    this.nextEl = container.querySelector('[data-id="next"]') as HTMLElement;
    this.gaugeFill = container.querySelector('[data-id="gauge"]') as HTMLElement;
  }

  render(progress: ProgressState): void {
    ...
    const next: FocusMilestone | undefined = MILESTONES.find(
      (m) => progress.verifiedCycles < m.cyclesRequired,
    );
    if (!next) {
