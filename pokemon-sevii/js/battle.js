...
// ---------- typed move particle effects ----------
const TYPE_FX = {
  fire:    { colors:['#f8e070','#f89030','#e84820'], mode:'rise',    spd:2.2, g:-0.05, life:22, size:3.2, count:16 },
  water:   { colors:['#a8d8f8','#5aa0e8','#3878c8'], mode:'burst',   spd:2.4, g:0.14,  life:22, size:3.0, count:16 },
  electric:{ colors:['#f8f070','#f8d030'],           mode:'zap',     spd:3.2, g:0,     life:12, size:3.0, count:14 },
  grass:   { colors:['#78c850','#48a038','#a0d860'], mode:'flutter', spd:1.6, g:0.05,  life:30, size:4.0, count:12 },
  ice:     { colors:['#c8f0f8','#a0e0f0','#ffffff'], mode:'burst',   spd:2.2, g:0.06,  life:22, size:2.6, count:16 },
  rock:    { colors:['#b09068','#8a6a48','#c8b090'], mode:'fall',    spd:2.4, g:0.24,  life:22, size:3.4, count:14 },
  ground:  { colors:['#c8a870','#a07840','#8a6030'], mode:'fall',    spd:2.2, g:0.24,  life:22, size:3.4, count:14 },
  poison:  { colors:['#c060c0','#a040a0','#e080e0'], mode:'rise',    spd:1.6, g:-0.02, life:26, size:3.0, count:14 },
  psychic: { colors:['#e080e0','#c060c0','#f8b0f8'], mode:'ring',    spd:2.2, g:0,     life:20, size:3.0, count:8  },
  ghost:   { colors:['#7050a0','#4a3868','#9070c0'], mode:'wisp',    spd:1.4, g:-0.03, life:28, size:4.0, count:12 },
  dark:    { colors:['#4a3850','#2e2440','#6a5078'], mode:'wisp',    spd:1.6, g:0,     life:24, size:4.0, count:12 },
  bug:     { colors:['#a8c040','#88a838','#c0d860'], mode:'burst',   spd:2.0, g:0.10,  life:20, size:3.0, count:14 },
  flying:  { colors:['#e8f0f8','#c8d8f0','#ffffff'], mode:'burst',   spd:2.8, g:0.02,  life:16, size:2.6, count:14 },
  fighting:{ colors:['#f8f0d0','#f8d048','#ffffff'], mode:'burst',   spd:3.0, g:0.05,  life:14, size:3.6, count:12, star:true },
  normal:  { colors:['#ffffff','#f8f0d0','#f8e8b0'], mode:'burst',   spd:2.6, g:0.05,  life:14, size:3.2, count:12, star:true },
  steel:   { colors:['#d8dce4','#a8b0bc','#ffffff'], mode:'burst',   spd:2.8, g:0.06,  life:16, size:2.6, count:14 },
  dragon:  { colors:['#7868c8','#a890f0','#c0b0f8'], mode:'burst',   spd:2.6, g:0.03,  life:20, size:3.2, count:14 },
};
function spawnMoveFx(type, side, boost){
  const def = TYPE_FX[type] || TYPE_FX.normal;
  const tx = side==='p' ? 338 : 120;   // enemy mon vs player mon centre
  const ty = side==='p' ? 100 : 200;
  const n = Math.round((def.count||14) * (boost||1));
  for(let i=0;i<n;i++){
    const a = Math.random()*Math.PI*2, sp = def.spd*(0.5+Math.random());
    let vx = Math.cos(a)*sp, vy = Math.sin(a)*sp;
    if(def.mode==='rise'){ vy = -Math.abs(vy)-1; vx*=0.6; }
    else if(def.mode==='fall'){ vy = Math.abs(vy)*0.6 - 1; }
    else if(def.mode==='ring'){ vx = Math.cos(a)*def.spd; vy = Math.sin(a)*def.spd*0.55; }
    B.view.fx.push({
      x: tx + (Math.random()*24-12), y: ty + (Math.random()*24-12), vx, vy,
      g: def.g||0, life: def.life, max: def.life, size: def.size,
      col: def.colors[i%def.colors.length], mode: def.mode,
      star: !!def.star, rot: Math.random()*6.28, spin: (Math.random()-0.5)*0.5,
    });
  }
}
function updateFx(){
  const fx = B.view.fx; if(!fx) return;
...
function drawFx(x){
  ...
  for(const p of fx){
    ...
    if(p.mode==='zap'){
      x.strokeStyle=p.col; x.lineWidth=1.8; x.lineCap='round';
      const s=p.size*1.8;
      x.beginPath(); x.moveTo(p.x-s,p.y-s*0.5); x.lineTo(p.x,p.y);
      x.lineTo(p.x-s*0.4,p.y+s*0.4); x.lineTo(p.x+s,p.y+s*0.2); x.stroke();
    } else if(p.mode==='ring'){
      x.strokeStyle=p.col; x.lineWidth=2; x.globalAlpha=a*0.85;
      const r=(p.max-p.life)*1.5+2; x.beginPath(); x.arc(p.x,p.y,r,0,7); x.stroke();
    } else if(p.mode==='flutter'){
      x.translate(p.x,p.y); x.rotate(p.rot); x.fillStyle=p.col;
      x.beginPath(); x.ellipse(0,0,p.size,p.size*0.42,0,0,7); x.fill();
    } else if(p.mode==='wisp'){
      x.fillStyle=p.col; x.globalAlpha=a*0.7;
      x.beginPath(); x.arc(p.x,p.y,p.size*(0.6+a*0.6),0,7); x.fill();
    } else if(p.star){
      x.translate(p.x,p.y); x.rotate(p.rot); x.fillStyle=p.col;
      const s=p.size;
      x.beginPath();
      for(let k=0;k<8;k++){ const rr=k%2?s*0.4:s; const aa=k*Math.PI/4;
        const px=Math.cos(aa)*rr, py=Math.sin(aa)*rr; k?x.lineTo(px,py):x.moveTo(px,py); }
      x.closePath(); x.fill();
    } else {
      x.fillStyle=p.col; x.beginPath(); x.arc(p.x,p.y,p.size,0,7); x.fill();
    }
    x.restore();
  }
}
...
async function bmsg(text, opts){
  ...
  const base = (typeof textRate==='function') ? textRate() : 1.7;
  while(B.textShown < total){
    B.textShown = Math.min(total, B.textShown + (Input.held('A')? base*2 : base));
...
async function useMove(user, target, slot, side){
  ...
  if(M.pow>0){
    ...
    for(let h=0; h<hits && target.hp>0; h++){
      ...
      spawnMoveFx(M.type, side, eff>1?1.6:eff<1?0.6:1);   // typed particle burst on target
...
Battle.draw = function(x){
  ...
  // move particle effects (above mons, below panels)
  drawFx(x);
